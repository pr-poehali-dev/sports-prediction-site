...
const forecastsData: Record<number, {
  id: number; sport: string; sportIcon: string; match: string;
  league: string; date: string; odds: string; confidence: number; price: number;
}> = {
  1: { id: 1, sport: "Футбол", sportIcon: "⚽", match: "Реал Мадрид — Барселона", league: "Ла Лига", date: "14 июня, 21:00", odds: "2.15", confidence: 87, price: 990 },
  2: { id: 2, sport: "Хоккей", sportIcon: "🏒", match: "ЦСКА — СКА", league: "КХЛ", date: "15 июня, 19:30", odds: "1.85", confidence: 79, price: 690 },
};
          ...
          <div className="md:col-span-2">
            <div className="rounded-2xl p-6 neon-border sticky top-24" style={{ backgroundColor: "#111420" }}>
              ...
              <div className="text-white/30 text-xs font-golos mb-5 flex items-center gap-1">
                <Icon name="Clock" size={11} /> {forecast.date}
              </div>
              <div className="mb-5">
            ...
            <div className="rounded-2xl p-6 neon-border" style={{ backgroundColor: "#111420" }}>
                  ...
                  {[{ key: "details", label: "Контакт" }, { key: "transfer", label: "Перевод" }].map((s, i) => (
                    <div key={s.key} className="flex items-center gap-3">
                      <div className="flex items-center gap-2">
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-oswald transition-all ${
                          step === s.key ? "bg-orange-500 text-white"
                          : step === "transfer" && s.key === "details" ? "bg-green-500 text-white"
              ...
              {step === "details" && (
                <div className="space-y-4">
                  ...
                  <button onClick={handleDetailsNext} className="btn-orange w-full py-3.5 rounded-xl text-sm uppercase tracking-wider">
                    Продолжить
                  </button>
                </div>
              )}
                  ...
                  >
                    {isLoading ? (
                      <span className="flex items-center justify-center gap-2">
                        <Icon name="Loader" size={18} className="animate-spin" /> Отправляем заявку...
                      </span>
                    ) : "✓ Я перевёл деньги"}
                  </button>
                ...
                <div className="text-center py-4">
                  ...
                  <div className="flex gap-3">
                    <Link to="/profile" className="flex-1">
                      <button className="btn-orange w-full py-3 rounded-xl text-sm uppercase tracking-wider">Личный кабинет</button>
                    </Link>
                    <Link to="/" className="flex-1">
                      <button className="w-full py-3 rounded-xl text-sm font-oswald uppercase tracking-wider border border-white/15 text-white/60 hover:border-white/30 hover:text-white/80 transition-all">На главную</button>
                    </Link>
                  </div>
                </div>
              )}
